...
fn auto_detect_config(config: PathBuf) -> PathBuf {
    if config == PathBuf::from("forge.toml") {
...
use forge_plugin_sdk_rust::{Capability, InvokeResult, PluginError, PluginServer};
...
impl forge_plugin_sdk_rust::Plugin for AuthPlugin {
    ...
    async fn invoke(&self, ctx: forge_plugin_sdk_rust::InvokeContext) -> InvokeResult {
...
#[forge_plugin_sdk_rust::async_trait]
impl forge_plugin_sdk_rust::Plugin for HealthPlugin {
    fn capabilities(&self) -> Vec<Capability> {
...
"#;
    ...
    let example_main = r##"use forge_plugin_sdk_rust::{Capability, InvokeResult, PluginError, PluginServer};

struct ExamplePlugin;

#[forge_plugin_sdk_rust::async_trait]
impl forge_plugin_sdk_rust::Plugin for ExamplePlugin {
    fn capabilities(&self) -> Vec<Capability> {
...
mod tests {
    ...
    fn new_plugin_creates_files() {
        ...
        assert!(cargo.contains("forge-plugin-sdk-rust"));
