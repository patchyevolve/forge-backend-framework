...
- **Initial-connection failure (gap, scheduled for v1.1):** If the kernel cannot connect to a plugin at startup (transport error, plugin not yet running, wrong address), the failure is logged and the plugin is abandoned in `CONNECTING` state — no retry is attempted. This differs from post-registration crashes (bullet 1 above), which DO trigger automatic restart with exponential backoff. The kernel currently requires all plugins to be reachable at startup; starting a plugin after the kernel requires a kernel restart. This limitation applies to Shape A (plugin-as-server) only — Shape B (managed subprocess) spawns the plugin itself and is not affected. Tracking issue: [`#42`](https://github.com/patchyevolve/forge-backend-framework/issues/42).
