...
use forge_plugin_sdk_rust::{Capability, InvokeContext, InvokeResult, KernelClient, Plugin, PluginError, PluginServer};
    ...
    async fn invoke(&self, ctx: InvokeContext) -> InvokeResult {
        match ctx.capability.as_str() {
            "forge.example.echo.middleware" => {
                ...
                let kernel = KernelClient::connect(KERNEL_GRPC_ADDR).await
...
async fn main() -> anyhow::Result<()> {
    ...
    let _addr = std::env::var("FORGE_LISTEN_ADDR")
        .unwrap_or_else(|_| "127.0.0.1:50063".into());
